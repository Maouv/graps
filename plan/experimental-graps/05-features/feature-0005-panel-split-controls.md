...
# Panel open and close split controls

> **Summary Block:** Control side-panel visibility using the retained split SVG assets and mirrored left behavior. This entity is planning-only and cannot enter implementation before its dependencies and Definition of Ready are satisfied.
...
### Acceptance Criteria
- [ ] Existing right split assets remain the source assets.
- [ ] Left control is mirrored in CSS/code, not duplicated.
- [ ] Every icon-only control has an aria-label and visible focus.
...
- Panel open and close split controls may violate its contract if dependent structural IDs, state, or fallback behavior is incomplete.
