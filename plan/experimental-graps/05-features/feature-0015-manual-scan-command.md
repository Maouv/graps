...
### Acceptance Criteria
- [ ] Invalid syntax returns concise help.
- [ ] Every mode returns structural results even if AI fails.
- [ ] Default project opening does not depend on manual `/scan`.
...
- Manual scan command may violate its contract if dependent structural IDs, state, or fallback behavior is incomplete.
